...
const createBlogPostPages = (config, blogPosts, globalHtmlTemplate, blogPostTemplate) => {
  blogPosts.forEach((post) => {
    const parsedPost = parseStaticPage(post, blogPosts, blogPostTemplate, config.getSiteUrl())
    fileManager.writeFile(`${config.getOutputFolder()}/${post.fileName}`, parsedPost)
  })
}
 ...
 *
 */ 
const build = (publishMode) => {
  config.setPublishMode(publishMode)
